...
message StreamingRecognizeResponse {
  ...
  enum SpeechEventType {
    ...
    // [Extension by Techmo]
    // This event indicates that the server has detected no speech input timeout
    // and expects no additional speech. Therefore, the server will not process
    // additional audio (although it may subsequently return additional results).
    // The client should stop sending additional audio data, half-close the gRPC
...
message StreamingRecognitionResult {
  ...
  // [Extension by Techmo]
  // *Optional* Detailed result (lattice).
  ...
  // When requested and not allowed by licence GRPC status: FAILED_PRECONDITION will be returned
...
message LatticeEdge {
  // input node id, node '0' is starting node for lattice
  int32 start_node = 1;
