...
cmd="${SCRIPTPATH}/../docker/run_dictation_client_cpp.sh --service-address=${test_service_address} --filename=${test_wave}"
phrase="serwis testowy"
check_output "${cmd}" "${phrase}"

echo
echo "Testing: --interim-results"
cmd="${SCRIPTPATH}/../docker/run_dictation_client_cpp.sh --service-address=${test_service_address} --filename=${test_wave} --interim-results"
phrase="serwis"
check_output "${cmd}" "${phrase}"

echo
echo "Testing: --grpc-timeout"
cmd="${SCRIPTPATH}/../docker/run_dictation_client_cpp.sh --service-address=${test_service_address} --filename=${test_wave} --grpc-timeout=50"
phrase="DEADLINE_EXCEEDED"
check_output "${cmd}" "${phrase}"

echo
echo "Testing: --time-offsets"
cmd="${SCRIPTPATH}/../docker/run_dictation_client_cpp.sh --service-address=${test_service_address} --filename=${test_wave} --time-offsets"
phrase="nanos: 60000000"
check_output "${cmd}" "${phrase}"

echo
echo "Testing: --context-phrase"
cmd="${SCRIPTPATH}/../docker/run_dictation_client_cpp.sh --service-address=${test_service_address} --filename=${test_context_wave} --context-phrase=context"
